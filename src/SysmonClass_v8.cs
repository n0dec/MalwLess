...
namespace MalwLess
{
	...
	public static class SysmonClass_v8
	{
		...
		static void writeErrorReport(JToken payload, JToken config){
			...
			if(!Sysmon_v8.Namespace.SYSMON_PROVIDER_V8.EventWriteSYSMON_ERROR_EVENT(UtcTime, ID, Description))
		...
		static void writeProcessCreate(JToken payload, JToken config){
			...
			if(!Sysmon_v8.Namespace.SYSMON_PROVIDER_V8.EventWriteSYSMON_CREATE_PROCESS_EVENT(RuleName, UtcTime, ProcessGuid, ProcessId, Image, FileVersion, Description, Product, Company, CommandLine, CurrentDirectory, User, LogonGuid, LogonId, TerminalSessionId, IntegrityLevel, Hashes, ParentProcessGuid, ParentProcessId, ParentImage, ParentCommandLine))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeFileCreateTime(JToken payload, JToken config){
			...
			if(!Sysmon_v8.Namespace.SYSMON_PROVIDER_V8.EventWriteSYSMON_FILE_TIME_EVENT(RuleName, UtcTime, ProcessGuid, ProcessId, Image, TargetFilename, CreationUtcTime, PreviousCreationUtcTime))
				Console.WriteLine("Error: Writing event");
		...
		static void writeNetworkConnect(JToken payload, JToken config){
			...
			if(!Sysmon_v8.Namespace.SYSMON_PROVIDER_V8.EventWriteSYSMON_NETWORK_CONNECT_EVENT(RuleName, UtcTime, ProcessGuid, ProcessId, Image, User, Protocol, Initiated, SourceIsIpv6, SourceIp, SourceHostname, SourcePort, SourcePortName, DestinationIsIpv6, DestinationIp, DestinationHostname, DestinationPort, DestinationPortName))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeServiceStateChanged(JToken payload, JToken config){
			...
			if(!Sysmon_v8.Namespace.SYSMON_PROVIDER_V8.EventWriteSYSMON_SERVICE_STATE_CHANGE_EVENT(UtcTime, State, Version, SchemaVersion))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeProcessTerminate(JToken payload, JToken config){
			...
			if(!Sysmon_v8.Namespace.SYSMON_PROVIDER_V8.EventWriteSYSMON_PROCESS_TERMINATE_EVENT(RuleName, UtcTime,ProcessGuid,ProcessId, Image))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeDriverLoad(JToken payload, JToken config){
			...
			if(!Sysmon_v8.Namespace.SYSMON_PROVIDER_V8.EventWriteSYSMON_DRIVER_LOAD_EVENT(RuleName, UtcTime, ImageLoaded,Hashes, Signed, Signature, SignatureStatus))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeImageLoad(JToken payload, JToken config){
			...
			if(!Sysmon_v8.Namespace.SYSMON_PROVIDER_V8.EventWriteSYSMON_IMAGE_LOAD_EVENT(RuleName, UtcTime, ProcessGuid, ProcessId, Image, ImageLoaded, FileVersion, Description, Product, Company, Hashes, Signed, Signature, SignatureStatus))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeCreateRemoteThread(JToken payload, JToken config){
			...
			if(!Sysmon_v8.Namespace.SYSMON_PROVIDER_V8.EventWriteSYSMON_CREATE_REMOTE_THREAD_EVENT(RuleName, UtcTime, SourceProcessGuid, SourceProcessId, SourceImage, TargetProcessGuid, TargetProcessId, TargetImage, NewThreadId, StartAddress, StartModule, StartFunction))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeRawAccessRead(JToken payload, JToken config){
			...
			if(!Sysmon_v8.Namespace.SYSMON_PROVIDER_V8.EventWriteSYSMON_RAWACCESS_READ_EVENT(RuleName, UtcTime, ProcessGuid, ProcessId, Image, Device))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeProcessAccess(JToken payload, JToken config){
			...
			if(!Sysmon_v8.Namespace.SYSMON_PROVIDER_V8.EventWriteSYSMON_ACCESS_PROCESS_EVENT(RuleName, UtcTime, SourceProcessGUID, SourceProcessId, SourceThreadId, SourceImage, TargetProcessGUID, TargetProcessId, TargetImage, GrantedAccess, CallTrace))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeFileCreate(JToken payload, JToken config){
			...
			if(!Sysmon_v8.Namespace.SYSMON_PROVIDER_V8.EventWriteSYSMON_FILE_CREATE_EVENT(RuleName, UtcTime, ProcessGuid, ProcessId, Image, TargetFilename, CreationUtcTime))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeRegistryEventRegKey(JToken payload, JToken config){
			...
			if(!Sysmon_v8.Namespace.SYSMON_PROVIDER_V8.EventWriteSYSMON_REG_KEY_EVENT(RuleName, EventType, UtcTime, ProcessGuid, ProcessId, Image, TargetObject))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeRegistryEventRegSetValue(JToken payload, JToken config){
			...
			if(!Sysmon_v8.Namespace.SYSMON_PROVIDER_V8.EventWriteSYSMON_REG_SETVALUE_EVENT(RuleName, EventType, UtcTime, ProcessGuid, ProcessId, Image, TargetObject, Details))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeRegistryEventRegName(JToken payload, JToken config){
			...
			if(!Sysmon_v8.Namespace.SYSMON_PROVIDER_V8.EventWriteSYSMON_REG_NAME_EVENT(RuleName, EventType, UtcTime, ProcessGuid, ProcessId, Image, TargetObject, NewName))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeFileCreateStreamHash(JToken payload, JToken config){
			...
			if(!Sysmon_v8.Namespace.SYSMON_PROVIDER_V8.EventWriteSYSMON_FILE_CREATE_STREAM_HASH_EVENT(RuleName, UtcTime, ProcessGuid, ProcessId, Image, TargetFilename, CreationUtcTime, Hash))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeServiceConfigurationChanged(JToken payload, JToken config){
			...
			if(!Sysmon_v8.Namespace.SYSMON_PROVIDER_V8.EventWriteSYSMON_SERVICE_CONFIGURATION_CHANGE_EVENT(UtcTime, Configuration, ConfigurationFileHash))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writePipeEventCreate(JToken payload, JToken config){
			...
			if(!Sysmon_v8.Namespace.SYSMON_PROVIDER_V8.EventWriteSYSMON_CREATE_NAMEDPIPE_EVENT(RuleName, UtcTime, ProcessGuid, ProcessId, PipeName, Image))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writePipeEventConnect(JToken payload, JToken config){
			...
			if(!Sysmon_v8.Namespace.SYSMON_PROVIDER_V8.EventWriteSYSMON_CONNECT_NAMEDPIPE_EVENT(RuleName, UtcTime, ProcessGuid, ProcessId, PipeName, Image))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeWmiEventFilter(JToken payload, JToken config){
			...
			if(!Sysmon_v8.Namespace.SYSMON_PROVIDER_V8.EventWriteSYSMON_WMI_FILTER_EVENT(RuleName, EventType, UtcTime, Operation, User, EventNamespace, Name, Query))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeWmiEventConsumer(JToken payload, JToken config){
			...
			if(!Sysmon_v8.Namespace.SYSMON_PROVIDER_V8.EventWriteSYSMON_WMI_CONSUMER_EVENT(RuleName, EventType, UtcTime, Operation, User, Name, Type, Destination))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeWmiEventBinding(JToken payload, JToken config){
			...
			if(!Sysmon_v8.Namespace.SYSMON_PROVIDER_V8.EventWriteSYSMON_WMI_BINDING_EVENT(RuleName, EventType, UtcTime, Operation, User, Consumer, Filter))
				Console.WriteLine("Error: Writing event");
		}
