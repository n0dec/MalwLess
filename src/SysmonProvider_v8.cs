...
namespace Sysmon_v8.Namespace
{
	using System;
	using System.Diagnostics.Eventing;
	using System.Runtime.InteropServices;

	public static class SYSMON_PROVIDER_V8
	{
		//
		...
		//
		public static bool EventWriteSYSMON_IMAGE_LOAD_EVENT(string RuleName, string UtcTime, Guid ProcessGuid, uint ProcessId, string Image, string ImageLoaded, string FileVersion, string Description, string Product, string Company, string Hashes, string Signed, string Signature, string SignatureStatus)
		{
			...
			return m_provider.TemplateImage_loaded(ref SYSMON_IMAGE_LOAD_EVENT, RuleName, UtcTime, ProcessGuid, ProcessId, Image, ImageLoaded, FileVersion, Description, Product, Company, Hashes, Signed, Signature, SignatureStatus);
		}
		...
		//
		public static bool EventWriteSYSMON_CREATE_NAMEDPIPE_EVENT(string RuleName, string UtcTime, Guid ProcessGuid, uint ProcessId, string PipeName, string Image)
		{
			...
			return m_provider.TemplatePipe_Created(ref SYSMON_CREATE_NAMEDPIPE_EVENT, RuleName, UtcTime, ProcessGuid, ProcessId, PipeName, Image);
		}

		//
		// Event method for SYSMON_CONNECT_NAMEDPIPE_EVENT
		//
		public static bool EventWriteSYSMON_CONNECT_NAMEDPIPE_EVENT(string RuleName, string UtcTime, Guid ProcessGuid, uint ProcessId, string PipeName, string Image)
		{
			...
			return m_provider.TemplatePipe_Connected(ref SYSMON_CONNECT_NAMEDPIPE_EVENT, RuleName, UtcTime, ProcessGuid, ProcessId, PipeName, Image);
		}
	...
	internal class EventProviderVersionTwo : EventProvider
	{
		...
		)
		{
			...
			if (IsEnabled(eventDescriptor.Level, eventDescriptor.Keywords))
			{
				...
				userDataPtr[9].Size = (uint)(CommandLine.Length + 1)*sizeof(char);

				userDataPtr[10].Size = (uint)(CurrentDirectory.Length + 1)*sizeof(char);

				userDataPtr[11].Size = (uint)(User.Length + 1)*sizeof(char);

				userDataPtr[12].DataPointer = (UInt64)(&LogonGuid);
				userDataPtr[12].Size = (uint)(sizeof(Guid)  );

				userDataPtr[13].DataPointer = (UInt64)(&LogonId);
				userDataPtr[13].Size = (uint)(sizeof(long)  );

				userDataPtr[14].DataPointer = (UInt64)(&TerminalSessionId);
				userDataPtr[14].Size = (uint)(sizeof(int)  );

				userDataPtr[15].Size = (uint)(IntegrityLevel.Length + 1)*sizeof(char);

				userDataPtr[16].Size = (uint)(Hashes.Length + 1)*sizeof(char);

				userDataPtr[17].DataPointer = (UInt64)(&ParentProcessGuid);
				userDataPtr[17].Size = (uint)(sizeof(Guid)  );

				userDataPtr[18].DataPointer = (UInt64)(&ParentProcessId);
				userDataPtr[18].Size = (uint)(sizeof(int)  );

				userDataPtr[19].Size = (uint)(ParentImage.Length + 1)*sizeof(char);

				userDataPtr[20].Size = (uint)(ParentCommandLine.Length + 1)*sizeof(char);

				fixed (char* a0 = RuleName, a1 = UtcTime, a2 = Image, a3 = FileVersion, a4 = Description, a5 = Product, a6 = Company, a7 = CommandLine, a8 = CurrentDirectory, a9 = User, a10 = IntegrityLevel, a11 = Hashes, a12 = ParentImage, a13 = ParentCommandLine)
				{
					userDataPtr[0].DataPointer = (ulong)a0;
					userDataPtr[1].DataPointer = (ulong)a1;
		...
		)
		{
			...
			if (IsEnabled(eventDescriptor.Level, eventDescriptor.Keywords))
			{
				...
				userDataPtr[10].Size = (uint)(Hashes.Length + 1)*sizeof(char);

				userDataPtr[11].Size = (uint)(Signed.Length + 1)*sizeof(char);

				userDataPtr[12].Size = (uint)(Signature.Length + 1)*sizeof(char);

				userDataPtr[13].Size = (uint)(SignatureStatus.Length + 1)*sizeof(char);

				fixed (char* a0 = RuleName, a1 = UtcTime, a2 = Image, a3 = ImageLoaded, a4 = FileVersion, a5 = Description, a6 = Product, a7 = Company, a8 = Hashes, a9 = Signed, a10 = Signature, a11 = SignatureStatus)
				{
					userDataPtr[0].DataPointer = (ulong)a0;
					userDataPtr[1].DataPointer = (ulong)a1;
					userDataPtr[4].DataPointer = (ulong)a2;
		...
		)
		{
			...
			if (IsEnabled(eventDescriptor.Level, eventDescriptor.Keywords))
			{
				...
				userDataPtr[4].Size = (uint)(PipeName.Length + 1)*sizeof(char);

				userDataPtr[5].Size = (uint)(Image.Length + 1)*sizeof(char);

				fixed (char* a0 = RuleName, a1 = UtcTime, a2 = PipeName, a3 = Image)
				{
					userDataPtr[0].DataPointer = (ulong)a0;
					userDataPtr[1].DataPointer = (ulong)a1;
					userDataPtr[4].DataPointer = (ulong)a2;
