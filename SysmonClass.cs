...
namespace MalwLess
{
	...
	public static class SysmonClass
	{
		...
		static void writeFileCreateTime(JToken payload, JToken config){
			...
			if(!Sysmon.Namespace.SYSMON_PROVIDER.EventWriteSYSMON_FILE_TIME_EVENT(UtcTime, ProcessGuid, ProcessId, Image, TargetFilename, CreationUtcTime, PreviousCreationUtcTime))
				Console.WriteLine("Error: Writing event");
		...
		static void writeNetworkConnect(JToken payload, JToken config){
			...
			if(!Sysmon.Namespace.SYSMON_PROVIDER.EventWriteSYSMON_NETWORK_CONNECT_EVENT(UtcTime, ProcessGuid, ProcessId, Image, User, Protocol, Initiated, SourceIsIpv6, SourceIp, SourceHostname, SourcePort, SourcePortName, DestinationIsIpv6, DestinationIp, DestinationHostname, DestinationPort, DestinationPortName))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeProcessTerminate(JToken payload, JToken config){
			...
			if(!Sysmon.Namespace.SYSMON_PROVIDER.EventWriteSYSMON_PROCESS_TERMINATE_EVENT(UtcTime,ProcessGuid,ProcessId, Image))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeDriverLoad(JToken payload, JToken config){
			...
			if(!Sysmon.Namespace.SYSMON_PROVIDER.EventWriteSYSMON_DRIVER_LOAD_EVENT(UtcTime, ImageLoaded,Hashes, Signed, Signature, SignatureStatus))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeImageLoad(JToken payload, JToken config){
			...
			if(!Sysmon.Namespace.SYSMON_PROVIDER.EventWriteSYSMON_IMAGE_LOAD_EVENT(UtcTime, ProcessGuid, ProcessId, Image, ImageLoaded, FileVersion, Description, Product, Company, Hashes, Signed, Signature, SignatureStatus))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeCreateRemoteThread(JToken payload, JToken config){
			...
			if(!Sysmon.Namespace.SYSMON_PROVIDER.EventWriteSYSMON_CREATE_REMOTE_THREAD_EVENT(UtcTime, SourceProcessGuid, SourceProcessId, SourceImage, TargetProcessGuid, TargetProcessId, TargetImage, NewThreadId, StartAddress, StartModule, StartFunction))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeRawAccessRead(JToken payload, JToken config){
			...
			if(!Sysmon.Namespace.SYSMON_PROVIDER.EventWriteSYSMON_RAWACCESS_READ_EVENT(UtcTime, ProcessGuid, ProcessId, Image, Device))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeProcessAccess(JToken payload, JToken config){
			...
			if(!Sysmon.Namespace.SYSMON_PROVIDER.EventWriteSYSMON_ACCESS_PROCESS_EVENT(UtcTime, SourceProcessGUID, SourceProcessId, SourceThreadId, SourceImage, TargetProcessGUID, TargetProcessId, TargetImage, GrantedAccess, CallTrace))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeFileCreate(JToken payload, JToken config){
			...
			if(!Sysmon.Namespace.SYSMON_PROVIDER.EventWriteSYSMON_FILE_CREATE_EVENT(UtcTime, ProcessGuid, ProcessId, Image, TargetFilename, CreationUtcTime))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeRegistryEventRegKey(JToken payload, JToken config){
			...
			if(!Sysmon.Namespace.SYSMON_PROVIDER.EventWriteSYSMON_REG_KEY_EVENT(EventType, UtcTime, ProcessGuid, ProcessId, Image, TargetObject))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeRegistryEventRegSetValue(JToken payload, JToken config){
			...
			if(!Sysmon.Namespace.SYSMON_PROVIDER.EventWriteSYSMON_REG_SETVALUE_EVENT(EventType, UtcTime, ProcessGuid, ProcessId, Image, TargetObject, Details))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeRegistryEventRegName(JToken payload, JToken config){
			...
			if(!Sysmon.Namespace.SYSMON_PROVIDER.EventWriteSYSMON_REG_NAME_EVENT(EventType, UtcTime, ProcessGuid, ProcessId, Image, TargetObject, NewName))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeFileCreateStreamHash(JToken payload, JToken config){
			...
			if(!Sysmon.Namespace.SYSMON_PROVIDER.EventWriteSYSMON_FILE_CREATE_STREAM_HASH_EVENT(UtcTime, ProcessGuid, ProcessId, Image, TargetFilename, CreationUtcTime, Hash))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writePipeEventCreate(JToken payload, JToken config){
			...
			if(!Sysmon.Namespace.SYSMON_PROVIDER.EventWriteSYSMON_CREATE_NAMEDPIPE_EVENT(UtcTime, ProcessGuid, ProcessId, PipeName, Image))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writePipeEventConnect(JToken payload, JToken config){
			...
			if(!Sysmon.Namespace.SYSMON_PROVIDER.EventWriteSYSMON_CONNECT_NAMEDPIPE_EVENT(UtcTime, ProcessGuid, ProcessId, PipeName, Image))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeWmiEventFilter(JToken payload, JToken config){
			...
			if(!Sysmon.Namespace.SYSMON_PROVIDER.EventWriteSYSMON_WMI_FILTER_EVENT(EventType, UtcTime, Operation, User, EventNamespace, Name, Query))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeWmiEventConsumer(JToken payload, JToken config){
			...
			if(!Sysmon.Namespace.SYSMON_PROVIDER.EventWriteSYSMON_WMI_CONSUMER_EVENT(EventType, UtcTime, Operation, User, Name, Type, Destination))
				Console.WriteLine("Error: Writing event");
		}
		...
		static void writeWmiEventBinding(JToken payload, JToken config){
			...
			if(!Sysmon.Namespace.SYSMON_PROVIDER.EventWriteSYSMON_WMI_BINDING_EVENT(EventType, UtcTime, Operation, User, Consumer, Filter))
				Console.WriteLine("Error: Writing event");
		}
